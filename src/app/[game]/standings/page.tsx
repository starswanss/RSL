...
import { getBrStandings, getBrFinalStandings, type BrStandingRow } from "@/lib/br";
...
function StandingsTable({
  ...
  rows: BrStandingRow[];
  slug: string;
  highlight: number; // จำนวนแถวบนสุดที่ไฮไลต์ (โซนผ่านเข้ารอบ) — 0 = ไม่ไฮไลต์
}) {
  return (
    <div className="rsl-card overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map((row, i) => (
              <tr key={row.teamId} className="border-b border-[color:var(--border)] last:border-0">
...
export default async function StandingsPage({
  ...
  const [groups, finals] = await Promise.all([
    getBrStandings(g.id),
    getBrFinalStandings(g.id),
  ]);
  const groupNames = Object.keys(groups).sort();
  const champion = finals.length > 0 && finals[0].matches > 0 ? finals[0] : null;
    ...
    <div>
        ...
        <section className="mb-10">
          <h3 className="text-lg font-bold mb-3">
            ...
            <span className="ml-2 text-xs font-normal text-[color:var(--text-dim)]">
              (คะแนนเริ่มนับใหม่ ไม่รวมรอบแบ่งสาย)
            </span>
          </h3>
          {champion && (
            <div className="rsl-card p-4 mb-3 inline-flex items-center gap-3">
              <span className="text-2xl">👑</span>
              <div>
                <p className="text-xs text-[color:var(--text-dim)]">ผู้นำรอบชิง</p>
                <p className="font-extrabold rsl-gradient-text text-lg">
                  {champion.name} · {champion.totalPoints} แต้ม
                </p>
              </div>
            </div>
          )}
          <StandingsTable rows={finals} slug={g.slug} highlight={1} />
        </section>
      )}
      ...
      {groupNames.length === 0 ? (
        ...
        <div className="space-y-8">
          <h3 className="text-lg font-bold">รอบแบ่งสาย</h3>
          {groupNames.map((grp) => (
            <div key={grp}>
              <h4 className="text-base font-bold mb-3">
                สาย <span className="rsl-gradient-text">{grp}</span>
              </h4>
              <StandingsTable rows={groups[grp]} slug={g.slug} highlight={2} />
            </div>
          ))}
